"""
きのこの山とたけのこの里を分類するための機械学習モデルを訓練し、テストするスクリプト。 temperature scalingを追加(gpu)

モデル生成に CPU てはなく GPU を使用するサンプルです。動作させるにはそれなりな準備が必要です。
...
def main():
    ...
    original_brier, original_ece = evaluate_calibration(model, test_loader, device)

    # Temperature Scaling の適用
    print("Calibrating temperature...")
    temperature_model = calibrate_temperature(model, val_loader, device)

    # Temperature Scaling 適用後のモデル評価
    print("Evaluating temperature-scaled model...")
    scaled_brier, scaled_ece = evaluate_calibration(temperature_model, test_loader, device)
    ...
    # Temperature Scaling 適用後のモデルの保存
    save_model(temperature_model, 'models/model_avoidance_temperature_scaled.pth')

    # テスト画像の予測
    predict_test_images(temperature_model, 'data/test', transform, device)

    # 提出用ファイルの作成
    create_submission(temperature_model, 'data/test', transform, device)
